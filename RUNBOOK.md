...
| REQ-14 | 1 | `await tick()` before `autosize.update()` — wait for Svelte DOM flush | Message disappeared but did not send (Enter + button). | `await tick()` added a microtask yield between clearing `newMessage` and the fetch. The form submit event fires during the yield gap, calling `sendMessage()` again with empty content. Reactivity shifts during tick, causing the in-flight fetch to land on stale component state. |
| REQ-14 | 2 | `dispatchEvent(new Event('input'))` — trigger autosize's own input listener synchronously | Both Enter and Send button stopped working entirely. | `bind:value` handler fires during dispatchEvent, triggers `disabled` re-evaluation on the submit button (`type="submit"` with `disabled={!newMessage.trim()}`). On the button click path, the re-enable causes the browser to fire a second submit on the same event cycle, which calls `sendMessage()` again with empty content. Both paths break because both trigger the same form onsubmit + disabled button cycle. |
| REQ-14 | 3 | Defer snap-back to after fetch — `inputRef.style.height = ''` after try/catch | Text submits (server OK) but does not appear in conversation view. | Root cause unidentified. Removing `autosize.update()` from the clear–fetch window may have exposed a pre-existing SSE or conversation state bug unrelated to snap-back. Pending further diagnosis. |
